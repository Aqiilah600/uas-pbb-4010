...
class EditMahasiswaPage extends ConsumerStatefulWidget {
  final Mahasiswa mahasiswa;

  const EditMahasiswaPage({super.key, required this.mahasiswa});

  @override
  ConsumerState<EditMahasiswaPage> createState() => _EditMahasiswaPageState();
}

class _EditMahasiswaPageState extends ConsumerState<EditMahasiswaPage> {
  ...
  late Set<int> _selectedHobiIds;

  @override
  void initState() {
    super.initState();
    final mhs = widget.mahasiswa;
    _namaController = TextEditingController(text: mhs.namaLengkap);
    _nimController = TextEditingController(text: mhs.nim);
    _tanggalController = TextEditingController(text: mhs.tanggalLahir);
    _angkatanController = TextEditingController(text: mhs.angkatan);
    _jenisKelamin = mhs.jenisKelamin;
    _selectedHobiIds = mhs.hobis.map((h) => h.id).toSet();
  }
  ...
  Future<void> _pickDate() async {
    final picked = await showDatePicker(
      context: context,
      initialDate: DateTime.tryParse(_tanggalController.text) ?? DateTime(2000),
